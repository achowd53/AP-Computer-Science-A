/******************************************************************************
 *  Compilation:  javac Terrain.java
 *  Execution:	java Terrain H iteration
 *
 *  Draws a wiremesh of a terrain using StdRandom and StdDraw with the Midpoint
 *  Displacement Method where iteration is the iteration for the Midpoint
 *  Displacement and H is the roughness of the terrain (H is a value between
 *  1.5 and 2 for the best results where the lower the H, the rougher the
 *  terrain, and the higher the terrain, the smoother it is).
 *
 *  % java Terrain 2 1
 *  Draws a figure divided into 4 connected polygons
 *
 *  % java Terrain 2 2
 *  Draws a figure divided into 16 connected polygons
 *
 *
******************************************************************************/
...
public class Terrain {
  ...
  //Outputs an array containing a list of the square roots of the number of points it takes to make every iteration from k to 1
  ...
  //Creates a grid with the number of points needed for iteration k, each point with an x and y coord
  public static double[][][] DefaultGrid(int k, int[] ld){
  double H = 2;
  StdDraw.setXscale(0,1);
  StdDraw.setYscale(0,1);
  int al = ld[0];
  double[][][] grid = new double[al+2][al+2][2];
...
//Connects all the points in the grid to display
...
//calculates displacement for midpoints
public static double d(double var, double H) {
  return StdRandom.gaussian(0, Math.sqrt(var))*Math.pow(.5,H);
}
//calculates average of 2 numbers
public static int mid(int a, int b) {
  return (a+b)/2;
}
//takes in 4 points of a quadrilateral, shifts the midpoints of each side and of the polygon, realligns inner points for when the
//polygon is split into 4 quadrants to repeat the process
public static void recSquare(double[][][] grid, int iteration, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, double H) {
  if (x2 - x1 == 1) return;
  //Figures out how much to displace each midpoint
  double var = Math.sqrt(1/Math.pow(2,2*H*(iteration+1)));
  double dc = d(var, H);
  double dt = d(var, H);
  double db = d(var, H);
  double dl = d(var, H);
  double dr = d(var, H);
  //Finds locations of midpoints
  ...
  //shifts midpoints in y direction
  grid[cp[0]][cp[1]][1] += dc;
  grid[tp[0]][tp[1]][1] += dt;
  grid[bp[0]][bp[1]][1] += db;
  grid[lp[0]][lp[1]][1] += dl;
  grid[rp[0]][rp[1]][1] += dr;
  //realligns grid for later iterations
  ...
  //Splits grid into 4 quadrants to repeat the process on each for
...
//Functions to reallign the grid for later iterations
