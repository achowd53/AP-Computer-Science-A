...
static final int color_switch = 0; //0 for color on, 1 for off
public static void GridConnection(double[][][] grid) {
  for (int y = 1; y < grid.length-2; y++) {
    for (int x = 1; x < grid.length-2; x++) {
      ...
      if (color_switch == 0) {
      float color_val = (float) ((grid[y][x][2] + grid[y+1][x][2] + grid[y+1][x+1][2] + grid[y][x+1][2]) / 4.0);
      Color color = Color.getHSBColor((float) color_val, 0.8f ,0.8f);
...
public static double d(double var, double H) {
  double out = StdRandom.gaussian(0,1) * var ;
  return out;
}
public static int mid(int a, int b) {
  return (a+b)/2;
}
public static void recSquare(double[][][] grid, int iteration, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, double H, double var) {
  if (x2 - x1 == 1) return;
  //not actual x and y points but column and row of grid point
  var *= Math.sqrt(1/Math.pow(2,2*H*(iteration+1))); //replace (2) with H later
  ...
  //time to reiterate
  recSquare(grid,iteration+1,x1,y1,tp[0],tp[1],cp[0],cp[1],lp[0],lp[1], H, var);
  recSquare(grid,iteration+1,tp[0],tp[1],x2,y2,rp[0],rp[1],cp[0],cp[1], H, var);
  recSquare(grid,iteration+1,cp[0],cp[1],rp[0],rp[1],x3,y3,bp[0],bp[1], H, var);
  recSquare(grid,iteration+1,lp[0],lp[1],cp[0],cp[1],bp[0],bp[1],x4,y4, H, var);
}
...
public static void main(String[] args) {
  ...
  recSquare(grid,1,1,ld[0],ld[0],ld[0],ld[0],1,1,1, H, var);
